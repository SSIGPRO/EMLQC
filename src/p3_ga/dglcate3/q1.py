...
def get_oracle():
    ...
    qc.append(adder, a[:]+b[:])                                 #b=b+a
    #qc.append(suber, c[:]+b[:])                                 #b=b-c
    qc.barrier() 
    ...
    #riscrivo il blocco sopra commentato per togliere la sottrzione:
    qc.compose(MCMTGate(ZGate(), 1, 1), b[:], inplace=True)
    qc.barrier()

    # uncompute f
    #qc.append(suber.inverse(), c[:]+b[:])                       #b=b+c
    qc.append(adder.inverse(), a[:]+b[:])                       #b=b-a
                                                                #alla fine si ha b=b(iniziale)
